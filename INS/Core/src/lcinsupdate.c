...
int8_t AddIMUData(const ImuData mImudata)
{
	...
	case INS_FUSING:
	{
		...
		{
			double d_compensente = sqrt(mUpdataStruct.X[0] * mUpdataStruct.X[0] + mUpdataStruct.X[1] * mUpdataStruct.X[1] + mUpdataStruct.X[2] * mUpdataStruct.X[2]);
			//if (d_compensente > 50)
			//{
	...
	if (mGnssInsSystem.mlc_STATUS == INS_FUSING)
	{
		...
		if (1)//mLCSetting.isUserOutPut Output result vechile system 
		{
			if (1 == mLCSetting.isusefliter)
			{
				positionfilter_compensent(&mGnssInsSystem.outNav, &mGnssInsSystem.posfliter, mGnssInsSystem.mPar.Rm, mGnssInsSystem.mPar.Rn, mGnssInsSystem.outNav.height, mGnssInsSystem.outNav.lat);
			}
